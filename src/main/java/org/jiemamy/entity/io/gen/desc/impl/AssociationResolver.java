...
public class AssociationResolver {
	...
	/** 多側に対する関連名のサフィックス */
	protected static final String TO_MANY_ASSOCIATION_NAME_SUFFIX = "List";
	 ...
	 */
	public AssociationResolver(EntitySetDesc entitySetDesc, PluralFormDictinary pluralFormDictinary) {
	...
	/**
	 * エンティティ記述のすべての属性について必要であれば名前を調整する。
	 * 
	 * @param fkMeta 外部キーメタデータ
	...
	protected void adjustAttributeNames(DbForeignKeyMeta fkMeta, EntityDesc entityDesc, String associationName) {
		...
		for (AttributeDesc attributeDesc : entityDesc.getAttributeDescList()) {
			if (associationName.equalsIgnoreCase(attributeDesc.getName())) {
				...
				String pkPropertyName = fromColumnNameToPropertyName(pkColumnName);
				String candidateName = attributeDesc.getName() + StringUtil.capitalize(pkPropertyName);
				String newName = toUniqueAttributeName(entityDesc, candidateName);
	 ...
	 * 
	 * @param fkMeta 外部キーメタデータ
	 * @param ownerEntityDesc 関連の所有者側のエンティティ記述
	 * @param inverseEntityDesc 関連の被所有者側のエンティティ記述
	 * @param mappedBy 関連の所有者側のプロパティ名
	 */
	protected void doInverseAssociationDesc(DbForeignKeyMeta fkMeta, EntityDesc ownerEntityDesc,
			EntityDesc inverseEntityDesc, String mappedBy) {
		AssociationDesc inverseAssociationDesc = new AssociationDesc();
		if (fkMeta.isUnique()) {
			String name = getInverseAssociationName(inverseEntityDesc, ownerEntityDesc, false);
			inverseAssociationDesc.setName(name);
			inverseAssociationDesc.setAssociationType(AssociationType.ONE_TO_ONE);
		} else {
			String name = getInverseAssociationName(inverseEntityDesc, ownerEntityDesc, true);
			inverseAssociationDesc.setName(name);
	 ...
	 * @param ownerEntityDesc 関連の所有者側のエンティティ記述
	 * @param inverseEntityDesc 関連の被所有者側のエンティティ記述
	 * @return {@link AssociationDesc}
	 */
	protected AssociationDesc doOwnerAssociationDesc(DbForeignKeyMeta fkMeta, EntityDesc ownerEntityDesc,
		...
		String name = getOwnerAssociationName(fkMeta, ownerEntityDesc, inverseEntityDesc);
	...
	private String fromColumnNameToPropertyName(String pkColumnName) {
	 ...
	 * 関連名を取得する。
	 * 
	 * @param inverseEntityDesc 参照する側のエンティティ記述
	 * @param ownerEntityDesc 参照される側のエンティティ記述
	 * @param oneToMany 関連がOneToManyの場合{@code true}
	 * @return 関連名
	 */
	protected String getInverseAssociationName(EntityDesc inverseEntityDesc, EntityDesc ownerEntityDesc,
			boolean oneToMany) {
		String associationName = StringUtil.decapitalize(ownerEntityDesc.getName());
		if (oneToMany) {
	 ...
	 * @return 関連の所有側の関連名
	 */
	protected String getOwnerAssociationName(DbForeignKeyMeta fkMeta, EntityDesc ownerEntityDesc,
			EntityDesc inverseEntityDesc) {
		String associationName = StringUtil.decapitalize(inverseEntityDesc.getName());
		if (!fkMeta.isComposite()) {
			...
			if (StringUtil.endsWithIgnoreCase(fkColumnName, pkColumnName)) {
				if (fkColumnName.length() > pkColumnName.length()) {
					...
					if (StringUtil.startsWithIgnoreCase(pkColumnName, inverseEntityDesc.getTableName())) {
						associationName = StringUtil.decapitalize(name) + inverseEntityDesc.getName();
					} else {
	...
	public void resolve(DbTableMeta tableMeta, DbForeignKeyMeta fkMeta) {
		EntityDesc ownerEntityDesc = entitySetDesc.getEntityDesc(tableMeta.getFullName());
		if (ownerEntityDesc == null) {
			return;
		}
		EntityDesc inverseEntityDesc = entitySetDesc.getEntityDesc(fkMeta.getPrimaryKeyFullTableName());
		if (inverseEntityDesc == null) {
			return;
		}
		AssociationDesc ownerAssociationDesc = doOwnerAssociationDesc(fkMeta, ownerEntityDesc, inverseEntityDesc);
		doInverseAssociationDesc(fkMeta, ownerEntityDesc, inverseEntityDesc, ownerAssociationDesc.getName());
	}
